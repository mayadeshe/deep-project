...
# ---------------------------------------------------------
# Improved DDPM: Vanilla + RePaint + negative prompt guidance
# ---------------------------------------------------------

@torch.no_grad()
def ddpm_inpaint_repaint(
        pipe,
        image: Image.Image,
        mask: torch.Tensor,
        prompt: str,
        ...
        seed: int,
        resample_steps: int = 5,  # RePaint iterations per timestep
):
    device = pipe.device
    ...
    # Text embeddings with negative prompt guidance
    ...
    for i, t in enumerate(timesteps):
        ...
        for r in range(resample_steps):
            ...
            # RePaint resampling (add noise jump to harmonize context)
...
# ---------------------------------------------------------
def main():
    parser = argparse.ArgumentParser("DDPM Inpainting + RePaint + Negative Prompt")
    parser.add_argument("--image", required=True)
    parser.add_argument("--mask", required=True)
    parser.add_argument("--prompt", required=True)
    parser.add_argument("--output_dir", default="output_repaint")
    parser.add_argument("--steps", type=int, default=50)
    parser.add_argument("--guidance_scale", type=float, default=7.5)
    parser.add_argument("--seed", type=int, default=42)
    parser.add_argument("--resample_steps", type=int, default=5,
                        help="RePaint resampling iterations per timestep. Default: 5.")
    ...
    print(f"Running DDPM inpainting with RePaint (resample_steps={args.resample_steps})...")
    result = ddpm_inpaint_repaint(
        pipe=pipe,
        image=image,
        mask=mask,
        prompt=args.prompt,
